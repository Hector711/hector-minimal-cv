...
export default function CalendlyButton({ children, className }: { children: any, className?: string }) {
  const eventName = "llamada-1-a-1";
  const backgroundColor = "292929";
  const textColor = "ffffff";
  const primaryColor = "16d378";
  const cookiesBanner = 1;

  const calendlyUrl = `https://calendly.com/hectorguerra_io/${eventName}?hide_gdpr_banner=${cookiesBanner}&background_color=${backgroundColor}&text_color=${textColor}&primary_color=${primaryColor}`;
